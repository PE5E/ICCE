...
#include <cstdint> // unit16_t // JB: didn't know there was such a thing ;-p

class DNA
{
    // JB: Enum! (And compute the value, if you can.)
    static const size_t d_charcap = 4;    // number of letters that can be stored
                                        // in 8 bits = 4, see also below
    static const char d_magicbyte = 255; // JB: Enums for magic, too.
    // JB: Naming of constants looks like ordinary data.

    std::ofstream   &d_os;  // JB: Loss of genericity by use of fstream.        
    std::ifstream   &d_is;         
...
// Binary format  // JB: Nice :-) I like clarifying comments.
// mapping:
//     00 A
//     01 C
//     10 G
